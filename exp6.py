...
def comp128v23_internal(KXOR,RAND):
	"""Internal part of the COMP128v23 algo
	"""
	temp = [0] * 16
	KM_RM = RAND + KXOR
...
def comp128v23(K, RAND, version = 2):
	...
	K = The secret Ki number
	RAND = The random number generated by the tower
	version = Version selecting integer
	"""
...
def hex2intarr(input):
	...
	"""
	# return list(map(lambda a: int(codecs.encode(bytes(a),'hex')), (a for a in codecs.decode(input,'hex'))))
	return [int(input[i:i+2],16) for i in range(0,len(input),2)]
...
if __name__ == '__main__':
	...
	parser.add_argument('Ki', metavar='Ki', default = "A34ABCDFACEAAE2987CCAABD54ADBBBB" ,nargs='?', help='The super secret Ki key')
